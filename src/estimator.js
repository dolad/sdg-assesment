const covid19ImpactEstimator = (data) => {
  ...
  const converttodays = (obj) => {
    ...
    if (obj.periodType === 'days'){ 
      return date = parseInt(obj.timeToElapse / 3);
    } else if (datas.periodType === 'week') {
      return date = parseInt((obj.timeToElapse * 7) / 3);
    } else if (obj.periodType === 'month') {
      return date = parseInt((obj.timeToElapse * 30) / 3);
    }
  };

  impact.infectionsByRequestedTime = 
    impact.currentlyInfected * (2 ** converttodays(data));
  severeImpact.infectionsByRequestedTime =
   severeImpact.currentlyInfected * (2 ** converttodays(data));
